...
flags.DEFINE_integer('ntest', 400, 'Number of testing examples')
flags.DEFINE_integer('ntrain', 2000, 'Number of training examples')
...
###############################################################################
###############################################################################
def get_imbalance_dataset(x_train,
                          y_train,
                          x_test,
                          y_test,
                          pos_ratio=0.9,
...
def evaluate(sess, x_, y_, acc_, train_set, test_set):
    ...
    acc_test_sum = 0.0
    train_bsize = 100
    for step in six.moves.xrange(2000 // train_bsize):
        x, y = train_set.next_batch(train_bsize)
        acc = sess.run(acc_, feed_dict={x_: x, y_: y})
        acc_sum += acc
    test_bsize = 100
    for step in six.moves.xrange(400 // test_bsize):
        x_test, y_test = test_set.next_batch(test_bsize)
        print ("Y test", y_test)
        acc = sess.run(acc_, feed_dict={x_: x_test, y_: y_test})
        print ("Acc test", acc)
        acc_test_sum += acc
    train_acc = acc_sum / float(2000 // train_bsize)
    test_acc = acc_test_sum / float(400 // test_bsize)
